...
export default function Navbar() {
    const { user, originalUser, stopImpersonating, logout } = useUser();
    ...
    if (user && user.role === "admin") {
        navLinks.push({ path: "/admin", label: "Admin Panel", icon: <ChartNoAxesColumnIcon size={18} /> });
    }

    return (
        <>
            {originalUser && (
                <div className="bg-amber-500 text-black py-2 px-4 text-center text-xs font-semibold flex items-center justify-center gap-2 z-[999] fixed top-0 w-full h-8">
                    <span className="truncate">Impersonating <strong>{user?.name}</strong> ({user?.email}). Actions will modify their records.</span>
                    <button onClick={stopImpersonating} className="bg-black text-amber-500 px-2.5 py-0.5 rounded-full hover:bg-black/80 transition-colors text-[10px] shrink-0 font-bold">
                        Exit Impersonation
                    </button>
                </div>
            )}
            <nav className={`fixed w-full bg-background/70 backdrop-blur-lg z-50 transition-all ${originalUser ? "top-8" : "top-0"}`}>
